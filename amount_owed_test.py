#!/usr/bin/env python3
"""
Backend Testing Script for Amount Owed Field Default Fix
...
def test_amount_owed_field_fix():
    """Test the amount_owed field default fix"""
    ...
    # Test 3: Verify clients via GET endpoint
    print("\n📋 TEST 3: Verify clients via GET /api/clients")
    try:
        ...
        if response.status_code == 200:
            ...
            for client in clients:
                if client.get('email') == 'unpaid_test@example.com':
                    unpaid_found = True
                    print(f"📋 Unpaid test client: amount_owed={client.get('amount_owed')}, monthly_fee={client.get('monthly_fee')}")
                    if client.get('amount_owed') == client.get('monthly_fee'):
                        test_results.append(("GET unpaid client verification", True, f"amount_owed={client.get('amount_owed')}"))
                    else:
                        test_results.append(("GET unpaid client verification", False, f"amount_owed={client.get('amount_owed')}, expected={client.get('monthly_fee')}"))
                
                elif client.get('email') == 'paid_test@example.com':
                    paid_found = True
                    print(f"📋 Paid test client: amount_owed={client.get('amount_owed')}, monthly_fee={client.get('monthly_fee')}")
                    if client.get('amount_owed') == 0.0:
                        test_results.append(("GET paid client verification", True, f"amount_owed={client.get('amount_owed')}"))
                    else:
                        test_results.append(("GET paid client verification", False, f"amount_owed={client.get('amount_owed')}, expected=0.0"))
            
            if not unpaid_found:
                test_results.append(("GET unpaid client verification", False, "Unpaid test client not found"))
            if not paid_found:
                test_results.append(("GET paid client verification", False, "Paid test client not found"))
                
        else:
            print(f"❌ Failed to get clients: {response.text}")
            test_results.append(("GET clients", False, f"Status: {response.status_code}"))
            
    except Exception as e:
        print(f"❌ Exception getting clients: {str(e)}")
        test_results.append(("GET clients", False, f"Exception: {str(e)}"))
    
    # Test 4: Test default behavior (no payment_status specified)
    print("\n📋 TEST 4: Create client without specifying payment_status (should default to 'due')")
    default_client_data = {
    ...
    for client_id, client_name in [(unpaid_client_id, "unpaid"), (paid_client_id, "paid"), (default_client_id, "default")]:
        if client_id:
            try:
