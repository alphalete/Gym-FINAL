  ...
  return (
    <div className="p-6">
      ...
      <div className="mb-6 p-4 bg-red-900 border border-red-600 rounded-lg">
        <h2 className="text-xl font-bold text-red-300 mb-4">🚨 EMERGENCY DIAGNOSTIC</h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
          <div className="bg-red-800 p-3 rounded">
            <p className="text-sm text-red-200">Total Clients:</p>
            <p className="text-2xl font-bold text-white">{clients.length}</p>
          </div>
          <div className="bg-red-800 p-3 rounded">
            <p className="text-sm text-red-200">Active Clients:</p>
            <p className="text-2xl font-bold text-white">{clients.filter(c => c.status === 'Active').length}</p>
          </div>
          <div className="bg-red-800 p-3 rounded">
            <p className="text-sm text-red-200">With Status:</p>
            <p className="text-2xl font-bold text-white">{clients.filter(c => c.status).length}</p>
          </div>
        </div>
        ...
        <div className="space-y-2">
          <button
            ...
            className="bg-red-600 hover:bg-red-700 px-4 py-2 rounded font-bold text-white mr-4"
          >
          ...
          <button
            ...
            className="bg-yellow-600 hover:bg-yellow-700 px-4 py-2 rounded font-bold text-white mr-4"
          >
            🔍 DIAGNOSTIC LOG
          </button>
          <button
            onClick={async () => {
              try {
                ...
                const sampleClients = [
                  ...
                  {
                    name: "EMERGENCY Sarah", 
                    email: "emergency.sarah@test.com",
                    phone: "(555) 999-0002",
                    membership_type: "Basic",
                    monthly_fee: 59.99,
                    start_date: "2025-07-15",
                ...
                for (const client of sampleClients) {
                  const result = await localDB.addClient(client);
                  console.log("Added emergency client:", result);
                }
                
                alert("EMERGENCY clients added! Refreshing...");
                fetchClients();
              } catch (error) {
                console.error("Error adding emergency clients:", error);
        ...
        {clients.length > 0 && (
          <div className="mt-4 text-sm text-red-200">
            <p className="font-bold">Current Clients:</p>
            {clients.map((client, index) => (
              <p key={index}>• {client.name} - Status: {client.status || 'UNDEFINED'} - Fee: ${client.monthly_fee}</p>
            ))}
          </div>
        )}
      </div>
