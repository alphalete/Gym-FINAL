...
import { PencilIcon, TrashIcon, ChatBubbleLeftRightIcon, BanknotesIcon, ClockIcon, ArrowRightIcon, EnvelopeIcon, ChevronDownIcon, ChevronRightIcon } from '@heroicons/react/24/outline';
...
const PaymentComponent = () => {
  ...
  async function handlePaymentSubmit() {
    ...
    // Update member with Alphalete Club payment logic including cycle restart
    ...
    const updated = { 
      ...
      start_date: alphaleteResult.newStartDate, // Update start date if cycle restarted
      ...
      cycleRestarted: alphaleteResult.cycleRestarted
    };
  ...
  const sendPaymentInvoiceEmail = async (member, payment, nextDueDate) => {
    try {
      ...
      const nextDueDateString = new Date(nextDueDate).toLocaleDateString('en-US', { 
        year: 'numeric', 
        month: 'long', 
        day: 'numeric' 
      });
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="px-6 py-6">
          <div className="flex items-center justify-between">
            <div>
              <h1 className="page-title text-indigo-600" id="payments-header">Payment Tracking</h1>
...
function MembershipManagement() {
  ...
  const handleDelete = async (plan) => {
    ...
    try {
      ...
      alert(`✅ Plan "${plan.name}" has been successfully deleted.`);
      
    } catch (error) {
      console.error('❌ Error deleting plan:', error);
      alert(`❌ Error deleting plan "${plan.name}". Please try again.\n\nError: ${error.message}`);
                ...
                try {
                  ...
                  setTimeout(() => {
                    ...
                    if (!window.closed) {
                      ...
                      // Also try to navigate away as fallback
                      setTimeout(() => {
                        try {
                          window.location.href = 'about:blank';
                        } catch (e) {
                          console.log('Cannot navigate away - user can close tab manually');
                        }
                      }, 3000);
                    }
                  }, 100);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (!nextDueDate || form.membershipType !== member.membership_type) {
        ...
        // Debug log to verify calculation  
        console.log('📅 EditMember - Start Date:', startDate.toISOString().slice(0, 10), 'Due Date:', nextDueDate, 'Days Added:', billingIntervalDays);
      }
