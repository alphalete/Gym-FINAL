...
export function promptPin({title="Enter PIN",subtitle="For security, please confirm your PIN to proceed."}={}){
    ...
    wrap.innerHTML=`<div class="absolute inset-0 bg-black/40"></div>
      <div class="relative bg-white rounded-2xl shadow-soft border w-[92%] max-w-sm p-5">
        ...
        <form class="mt-4 space-y-3">
          <input type="password" inputmode="numeric" autocomplete="one-time-code" placeholder="••••" class="w-full rounded-xl border px-3 py-2" id="__pin_input" />
...
export async function requirePinIfEnabled(actionName="action"){
  ...
  if(!ok){ alert("Incorrect PIN."); return false; }
  return true;
}