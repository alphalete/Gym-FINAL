...
class OptimisticToggleBackendTest:
    ...
    async def test_backend_readiness(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/health") as response:
                if response.status == 200:
                    ...
                    self.log_test("Backend Health Check", False, f"Health endpoint returned {response.status}")
                    return False
                    
            # Test membership types endpoint availability
            async with self.session.get(f"{self.base_url}/membership-types") as response:
                if response.status == 200:
                    membership_types = await response.json()
                    ...
                    self.log_test("Membership Types Endpoint", False, f"Endpoint returned {response.status}")
                    return False
    ...
    async def test_rapid_toggle_handling(self):
        ...
        try:
            ...
            async with self.session.post(f"{self.base_url}/membership-types", 
                                       data=json.dumps(test_plan)) as response:
                if response.status == 200:
                    ...
                    for i in range(5):
                        ...
                        async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                                  data=json.dumps(update_data)) as update_response:
                            response_time = time.time() - start_time
                            
                            if update_response.status == 200:
                                updated_plan = await update_response.json()
                    ...
                    for i in range(3):
                        ...
                        task = self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                    ...
                    for i, response in enumerate(concurrent_responses):
                        if isinstance(response, Exception):
                            self.log_test(f"Concurrent Update {i+1}", False, f"Exception: {str(response)}")
                            concurrent_success = False
                        else:
                            if response.status == 200:
                                self.log_test(f"Concurrent Update {i+1}", True, "Update successful")
                            else:
                                self.log_test(f"Concurrent Update {i+1}", False, f"Status: {response.status}")
                                concurrent_success = False
                            response.close()
                            
                    self.log_test("Concurrent Updates Handling", concurrent_success,
                                "Backend handled concurrent updates properly")
                    
                else:
                    self.log_test("Test Plan Creation", False, f"Failed to create test plan: {response.status}")
                    return False
    ...
    async def test_plans_store_performance(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/membership-types") as response:
                ...
                if response.status == 200:
                    ...
                    self.log_test("Plans Store Performance", False, f"Failed to retrieve plans: {response.status}")
                    return False
    ...
    async def test_data_consistency(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/membership-types") as response:
                if response.status == 200:
                    initial_plans = await response.json()
                        ...
                        async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                            ...
                            if update_response.status == 200:
                                ...
                                if updated_plan.get('is_active') == new_active:
                                    ...
                                    async with self.session.get(f"{self.base_url}/membership-types") as list_response:
                                        if list_response.status == 200:
                                            updated_list = await list_response.json()
                                            ...
                                            self.log_test("List Endpoint Consistency", False,
                                                        f"List endpoint failed: {list_response.status}")
                                    ...
                                    async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                                              data=json.dumps(restore_data)) as restore_response:
                                ...
                                self.log_test("Data Consistency Update", False, f"Update failed: {update_response.status}")
                    else:
                        self.log_test("Data Consistency", False, "No plans available for testing")
                        return False
                else:
                    self.log_test("Data Consistency", False, f"Failed to get plans: {response.status}")
                    return False
    ...
    async def test_indexeddb_integration(self):
            ...
            async with self.session.get(f"{self.base_url}/membership-types") as response:
                ...
                if response.status == 200:
                    ...
                    if plans:
                        ...
                        async with self.session.get(f"{self.base_url}/membership-types/{test_plan_id}") as single_response:
                            if single_response.status == 200:
                                single_plan = await single_response.json()
                                ...
                                self.log_test("Individual Plan Retrieval", False,
                                            f"Failed to retrieve individual plan: {single_response.status}")
                    ...
                    if plans:
                        ...
                        async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                            ...
                            if update_response.status == 200:
                                ...
                                await self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                    ...
                    self.log_test("IndexedDB Integration Support", False, f"Bulk retrieval failed: {response.status}")
                    return False
    ...
    async def test_rollback_support(self):
        ...
        try:
            ...
            async with self.session.get(f"{self.base_url}/membership-types") as response:
                if response.status == 200:
                    ...
                    if plans:
                        ...
                        async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                            ...
                            if update_response.status == 200:
                                ...
                                async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                                          data=json.dumps(rollback_data)) as rollback_response:
                                    if rollback_response.status == 200:
                                        rolled_back_plan = await rollback_response.json()
                                        ...
                                        self.log_test("Rollback Operation", False,
                                                    f"Rollback failed: {rollback_response.status}")
                            else:
                                self.log_test("Initial Update for Rollback Test", False,
                                            f"Initial update failed: {update_response.status}")
                        ...
                        async with self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                                  data=json.dumps(invalid_update_data)) as invalid_response:
                            
                            # Check if backend properly rejects invalid data
                            if invalid_response.status in [400, 422]:  # Bad request or validation error
                                self.log_test("Invalid Update Rejection", True,
                                            f"Backend properly rejected invalid data with status {invalid_response.status}")
                                
                                # Verify original data is unchanged
                                async with self.session.get(f"{self.base_url}/membership-types/{plan_id}") as verify_response:
                                    if verify_response.status == 200:
                                        current_plan = await verify_response.json()
                                ...
                                self.log_test("Invalid Update Rejection", False,
                                            f"Backend accepted invalid data with status {invalid_response.status}")
                        ...
                        task1 = self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                               data=json.dumps(update1_data))
                        task2 = self.session.put(f"{self.base_url}/membership-types/{plan_id}",
                                               data=json.dumps(update2_data))
                        ...
                        async with self.session.get(f"{self.base_url}/membership-types/{plan_id}") as final_response:
                            if final_response.status == 200:
                                final_plan = await final_response.json()
                    ...
                    self.log_test("Rollback Support", False, f"Failed to get plans: {response.status}")
                    return False
    ...
    async def cleanup_test_data(self):
        ...
        for plan_id in self.test_membership_types:
            try:
                async with self.session.delete(f"{self.base_url}/membership-types/{plan_id}") as response:
                    if response.status == 200:
                        print(f"✅ Cleaned up test plan: {plan_id}")
                    else:
                        print(f"⚠️ Could not clean up test plan {plan_id}: {response.status}")
            except Exception as e:
